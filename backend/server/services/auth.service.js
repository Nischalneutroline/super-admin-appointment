...
export const registerSuperAdmin = async (req, res) => {
  ...
  if (error) {
    console.log("Th error in the validation", error);
  ...
  try {
    ...
    if (password !== confirmPassword) {
      ...
      console.log(
        "entered the if block to check the password and confirm password"
      );
      return res.status(400).json(
        createResponse({
          isSuccess: false,
...
export const loginSuperAdmin = async (req, res) => {
  ...
  try {
    ...
    console.log("The req body in login ", req.body);
